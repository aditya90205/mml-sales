const STORAGE_KEY = "mml_hrms_resignations";
...
export const CLEARANCE_ITEMS = [
  { key: "manager", label: "Manager sign-off" },
  { key: "it", label: "IT assets returned" },
  { key: "finance", label: "Finance settlement" },
  { key: "admin", label: "Admin / ID card handover" },
];
...
const DEFAULT_RESIGNATIONS = [
  {
    ...
    timeline: [
      { status: "Submitted", date: "2026-07-12", note: "Resignation submitted by employee." },
      { status: "Under Review", date: "2026-07-13", note: "Reviewed by reporting manager." },
      { status: "Approved", date: "2026-07-15", note: "Approved. Notice period confirmed at 30 days.", by: "Komal Mehra (HR)" },
      { status: "Notice Period", date: "2026-07-15", note: "Now serving notice period.", by: "Komal Mehra (HR)" },
    ],
  },
  {
    id: "r2",
    ...
    timeline: [
      { status: "Submitted", date: "2026-06-02", note: "Resignation submitted by employee." },
      { status: "Under Review", date: "2026-06-03", note: "Reviewed by reporting manager." },
      { status: "Approved", date: "2026-06-04", note: "Approved.", by: "Komal Mehra (HR)" },
      { status: "Notice Period", date: "2026-06-04", note: "Serving notice period.", by: "Komal Mehra (HR)" },
      { status: "Clearance", date: "2026-06-30", note: "All clearances completed.", by: "Komal Mehra (HR)" },
      { status: "Completed", date: "2026-07-02", note: "Exit process completed. Full and final settlement done.", by: "Komal Mehra (HR)" },
    ],
  },
  {
    id: "r3",
    ...
    hrNote: "",
    timeline: [
      { status: "Submitted", date: "2026-08-20", note: "Resignation submitted by employee." },
    ],
  },
];
...
export function submitResignation({ employeeName, employeeId, department, designation, requestedLastDay, noticePeriodDays, reason, reasonDetails }) {
  ...
  const record = {
    ...
    clearance: { manager: false, it: false, finance: false, admin: false },
    hrNote: "",
    timeline: [{ status: "Submitted", date: todayISO(), note: "Resignation submitted by employee." }],
  };
  ...
  const next = list.map((r) =>
    r.id === id
      ? {
          ...r,
          status: "Withdrawn",
          timeline: [...r.timeline, { status: "Withdrawn", date: todayISO(), note: "Resignation withdrawn by employee." }],
        }
      : r
  );
