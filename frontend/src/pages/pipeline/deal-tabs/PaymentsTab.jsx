...
import { dashRow, dashRows, EMPTY } from "./stageContent.jsx";
...
function InvoiceCell({ invoice, empty = false, onShared }) {
  if (empty || !invoice) {
    return <span className="text-[#9CA3AF]">{EMPTY}</span>;
  }
...
export default function PaymentsTab({ locked = true, empty = false }) {
  ...
  const { sorted, sort, toggle } = useTableSort(dashRows(rows, empty, ["mode", "invoice"]), { defaultKey: "date" });
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <OutlineButton
            onClick={() => toast.success("Downloading consolidated invoice PDF...")}
          >
        ...
        <DeskTable columns={COLUMNS} sort={sort} onSort={toggle}>
          {sorted.map((row, i) => (
            <tr key={`${row.reference}-${i}`} className="border-b border-black/5 last:border-0">
              ...
              <Td>
                <InvoiceCell invoice={row.invoice} empty={empty} onShared={markInvoiceShared} />
              </Td>
            </tr>
          ))}
        </DeskTable>
